...
@ActiveProfiles("doma")
@Transactional
internal class PurchaseOperationServiceDomaTest {
    @Autowired
    private val purchaseOperationService: PurchaseOperationService? = null

    @Autowired
    private val purchaseInvoiceDao: PurchaseInvoiceDao? = null

    @Autowired
    private val purchaseInvoiceDetailDao: PurchaseInvoiceDetailDao? = null

    @Autowired
    private val bookStockDao: BookStockDao? = null
    ...
    @Test
    fun usesDomaAsPrimaryPurchaseService() {
        Assertions.assertThat(purchaseOperationService)
            .isInstanceOf(PurchaseOperationServiceDoma::class.java)
    }
    ...
    fun createReturnsResponseAndPersistsPurchaseInvoiceDetailsAndAddsExistingStock() {
        ...
        val purchaseInvoice = purchaseInvoiceDao!!.selectById(response.id)
        ...
        response.detail!!.forEach(Consumer { detail: PurchaseInvoiceDetailResponse? ->
            val persistedDetail = purchaseInvoiceDetailDao!!.selectById(detail!!.id)
        ...
        Assertions.assertThat(bookStockDao!!.selectById(1L).getBookStockQuantity()).isEqualTo(12)
        Assertions.assertThat(bookStockDao.selectById(4L).getBookStockQuantity()).isEqualTo(14)
    ...
    fun createInsertsBookStockWhenStockDoesNotExist() {
        ...
        val bookStock = bookStockDao!!.selectById(stockId)
