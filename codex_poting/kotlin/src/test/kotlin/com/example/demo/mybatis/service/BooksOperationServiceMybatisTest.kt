...
@Transactional
internal class BooksOperationServiceMybatisTest {
    @Autowired
    private val bookService: BooksOperationServiceMybatis? = null
    ...
    fun findByIdThrowsWhenBookDoesNotExist() {
        Assertions.assertThatThrownBy(ThrowingCallable { bookService!!.findById(999L) })
            .isInstanceOf(RepositoryDataNotfoundException::class.java)
    }

    @Test
    fun searchIgnoresCase() {
        val books = bookService!!.search("spring", null, null, 0, 10)
    ...
    fun searchMatchesAuthorStartingWithIgnoreCase() {
        val books = bookService!!.search("taro", null, null, 0, 10)
    ...
    fun searchDoesNotMatchTitleContainingKeyword() {
        val books = bookService!!.search("入門", null, null, 0, 10)
    ...
    fun searchDoesNotMatchAuthorContainingKeyword() {
        val books = bookService!!.search("aro", null, null, 0, 10)
    ...
    fun searchFiltersByReleaseDateRange() {
        val books = bookService!!.search("spring", LocalDate.of(2020, 1, 1), LocalDate.of(2020, 1, 1), 0, 10)
    ...
    fun searchDoesNotFilterByKeywordWhenKeywordIsNull() {
        val books = bookService!!.search(null, null, null, 0, 2)
    ...
    fun searchDoesNotFilterByKeywordWhenKeywordIsBlank() {
        val books = bookService!!.search("   ", null, null, 0, 2)
    ...
    fun searchReturnsFirstPageAndMetadata() {
        val books = bookService!!.search("はじめて", null, null, 0, 2)
    ...
    fun searchReturnsSecondPage() {
        val books = bookService!!.search("はじめて", null, null, 1, 2)
    ...
    fun findByIdReturnsEmptyBookStockListWhenBookHasNoStock() {
        val book = bookService!!.findById(6L)
    ...
    fun searchReturnsEmptyContentWhenPageOutOfRange() {
        val firstPage = bookService!!.search("はじめて", null, null, 0, 2)
        val books = bookService.search("はじめて", null, null, firstPage.totalPages, 2)
    ...
    fun searchAppliesReleaseDateRangeWithPaging() {
        val books = bookService!!.search("はじめて", LocalDate.of(2020, 2, 1), LocalDate.of(2020, 2, 1), 0, 3)
    ...
    fun createReturnsGeneratedIdAndResponse() {
        ...
        val book =
            bookService!!.create(BookCreateRequest("MyBatis入門", "Jiro", releaseDate, 2L, 5L, "9784000000201", 1400))

        Assertions.assertThat(book!!.id).isNotNull()
        Assertions.assertThat(book.title).isEqualTo("MyBatis入門")
        ...
        Assertions.assertThat(book.genreId).isEqualTo(5L)
        Assertions.assertThat(book.genreName).isEqualTo("工学")
        Assertions.assertThat(book.isbn).isEqualTo("9784000000201")
    ...
    fun createSalesUnitPriceSchedulesFuturePrice() {
        ...
        bookService!!.createSalesUnitPrice(1L, BookSalesUnitPriceCreateRequest(1500, effectiveFrom))

        Assertions.assertThat(bookService.findById(1L)!!.salesUnitPrice).isEqualTo(1200)
        Assertions.assertThatThrownBy(ThrowingCallable {
            bookService.createSalesUnitPrice(
                1L,
    ...
    fun updateChangesBookAndUpdateAt() {
        val before = bookService!!.findById(1L)
        ...
        Assertions.assertThat(updated!!.title).isEqualTo("MyBatis更新")
        ...
        Assertions.assertThat(updated.isbn).isEqualTo("9784000000211")
    ...
    fun deleteRemovesBook() {
        ...
        Assertions.assertThatThrownBy(ThrowingCallable { bookService.findById(1L) })
            .isInstanceOf(RepositoryDataNotfoundException::class.java)
    }
    ...
    fun deleteThrowsWhenWriteLockCannotBeAcquired() {
        BookRowLock.acquire(dataSource!!, 1L).use { ignored ->
            Assertions.assertThatThrownBy(ThrowingCallable { bookService!!.delete(1L) })
                .isInstanceOf(PessimisticLockingFailureException::class.java)
        }
    }
