...
- `BooksOperationService` は MyBatis / Doma / jOOQ 共通の Service インターフェースです。
- `PurchaseOperationService` は MyBatis / Doma / jOOQ 共通の仕入登録 Service インターフェースです。
- `BookDataValidatorMybatis` / `BookDataValidatorDoma` / `BookDataValidatorJooq` は永続化方式ごとの本データバリデーションを扱います。
- `PurchaseDataValidatorMybatis` / `PurchaseDataValidatorDoma` / `PurchaseDataValidatorJooq` は永続化方式ごとの仕入データバリデーションを扱います。
- `BookOperationConverterMybatis` / `BookOperationConverterDoma` / `BookOperationConverterJooq` は表示向け Entity / row から response DTO への変換を扱います。jOOQ の行データは converter で書籍単位に集約し、`salesUnitPrice` と `bookStockList` を組み立てます。
- `PurchaseOperationConverterMybatis` / `PurchaseOperationConverterDoma` / `PurchaseOperationConverterJooq` は仕入登録用の Entity / row、在庫増減履歴、金額計算、response DTO への変換を扱います。
...
- `purchase_invoice.purchase_invoice_type` は `PurchaseInvoiceType` で扱います。MyBatis / Doma / jOOQ の型変換設定を揃えてください。
- `book_stock_movement.movement_type` は `BookStockMovementType`、`book_stock_movement.source_type` は `BookStockMovementSourceType` で扱います。MyBatis / Doma / jOOQ の型変換または値変換の設定を揃えてください。
- この Kotlin 版では JPA を扱いません。JPA profile、Spring Data JPA repository、JPA Entity、JPA converter、JPA test は追加しないでください。
...
- MyBatis / Doma / jOOQ のうち1つの実装を変更する場合でも、他の実装で同じ仕様が必要か確認してください。
...
- 手書きの永続化実装は Kotlin で管理します。`src/main/kotlin/com/example/demo/mybatis/generator`、`doma/generator`、`jooq/generated`、`openbd/generated` 配下の Java は生成コードとして扱い、直接編集しないでください。

## jOOQ

- `src/main/kotlin/com/example/demo/jooq` 配下は jOOQ 手書き実装です。
- jOOQ 生成コードは `src/main/kotlin/com/example/demo/jooq/generated` 配下に生成します。
...
- `compileKotlin` と `kaptGenerateStubsKotlin` は `generateJooq` と `syncOpenBdGeneratedSources` に依存しています。通常のビルドでも jOOQ 生成コードや OpenBD 生成コードが更新される可能性があるため、生成差分を確認してください。
...
- `src/main/kotlin/com/example/demo/doma/generator` 配下は Doma CodeGen の生成コードです。手作業での編集は避けてください。
...
- Kotlin で Doma の `@AggregateStrategy` / `@AssociationLinker` を定義する場合、Doma processor から見える Java static field にする必要があります。companion object の linker property には `@JvmField` を付けてください。
...
- `BookStockCustomDao/selectByStoreIdAndBookIdWithWriteLock.sql` は仕入登録時の在庫加算で使います。ロック方針を変更する場合は MyBatis / jOOQ 側も揃えてください。
...
- `src/main/kotlin/com/example/demo/mybatis/generator` 配下は MyBatis Generator の生成コードです。手作業での編集は避けてください。
...
- `BookStockCustomMapper.selectByStoreIdAndBookIdWithWriteLock` は仕入登録時の在庫加算で使います。ロック方針を変更する場合は Doma / jOOQ 側も揃えてください。
...
- 共有ドメイン型は `src/main/kotlin/com/example/demo/data/domain` 配下に置きます。
- Doma `@Domain(factoryMethod = "of")` を Kotlin enum / class に付ける場合、factory method は Java static method として見える必要があります。`of` には `@JvmStatic` を付けてください。
...
- `schema.sql` は通常テストや起動時の SQL 初期化で使われます。`generator-schema.sql` は MyBatis Generator、Doma CodeGen、jOOQ CodeGen で使われます。スキーマ変更時は両方の整合性を確認してください。
- `release_date` は検索条件と DTO に関係します。変更時は API バリデーションと3つの Service 実装を確認してください。
- `isbn` は `book` の一意制約、`@Isbn`、request / response DTO、仕入明細の本参照、各 `BookDataValidator*` / `PurchaseDataValidator*` に関係します。変更時は3つの Service 実装と `GlobalExceptionHandler` の `UniqueConstraintValidationException` も確認してください。
- 永続化方式ごとのデータバリデーションを変更する場合は、MyBatis の `BookDataValidatorMybatisTest` / `PurchaseDataValidatorMybatisTest`、Doma の `PurchaseDataValidatorDomaTest`、jOOQ の `BookDataValidatorJooqTest` / `PurchaseDataValidatorJooqTest` を確認してください。
- `publisher_id` は `publisher`、`genre_id` は `book_genre` への外部キーです。変更時は初期データ、生成 Mapper/DAO、外部キー参照チェックを確認してください。
- `publisher_name` / `genre_name` はレスポンス表示項目です。変更時は表示向け Entity / row / SQL / query / `BookOperationConverter*` を確認してください。
- `salesUnitPrice` は `BookResponse` のレスポンス表示項目で、`book_sales_unit_price_history` の現在有効な履歴から取得します。変更時は request / response DTO、MyBatis / Doma の `BookWithPublisherName`、jOOQ `BookWithStockRow`、取得・検索 SQL / query、検索 count、`BookOperationConverter*` を確認してください。
- 販売単価履歴を追加する API は `BookSalesUnitPriceCreateRequest` を受け取り、`book_sales_unit_price_history` の `book_id,effective_from` 一意制約、`sales_unit_price` の CHECK 制約、`effective_from` / `effective_to` の期間整合性に関係します。変更時は MyBatis / Doma / jOOQ の前後履歴更新処理を揃えてください。
- `bookStockList` はレスポンス表示項目です。`book_stock` または `store` を変更する場合は、MyBatis の nested collection、Doma の aggregate strategy、jOOQ の `BookWithStockRow`、`BookStockResponse`、`BookOperationConverter*` を確認してください。
...
- 検索条件を変更する場合は、MyBatis / Doma / jOOQ の一覧取得と件数取得が同じ条件になるよう確認してください。
...
jOOQ 生成コードは `src/main/kotlin/com/example/demo/jooq/generated` 配下に出力されます。OpenBD API クライアント生成コードは `src/main/kotlin/com/example/demo/openbd/generated` 配下に同期されます。`compileKotlin` と `kaptGenerateStubsKotlin` は `generateJooq` と `syncOpenBdGeneratedSources` に依存しているため、通常のビルド後にも差分を確認してください。
