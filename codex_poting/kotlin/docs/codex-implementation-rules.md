...
- このプロジェクトは Java Spring ORM Sample を Kotlin 化した JPA なしのサンプルとして扱う。
- JPA の実装、設定、依存関係、テストを追加しない。
- Lombok は使わない。Kotlin の property、data class、primary constructor injection、nullability を優先する。
- 手書きコードは Kotlin で実装する。`src/main/kotlin` / `src/test/kotlin` 配下に残る Java は `generated` / `generator` 配下の生成コードのみ許容する。
- 既存の Java 由来コードを Kotlin 化する場合は、動作を変えずに小さく置き換える。J2K 後は JavaBean getter/setter、`String.formatted`、`List.getFirst()`、Mockito matcher、Kotlin annotation target などの機械変換崩れを確認する。
- `LocalDateTime.now()` を頻繁に呼ばず、同じ処理内では必要に応じて値を使い回す。
- API のリクエスト、レスポンス項目にある `List` には `@NotNull` を付ける。
- 未使用なメソッドであれば削除する。
- MyBatis / Doma / jOOQ で共通利用する値オブジェクトや列挙型は `src/main/kotlin/com/example/demo/data/domain` 配下に置く。
- 共有ドメイン型を DB に保存する場合は、MyBatis TypeHandler、Doma `@Domain`、jOOQ 側の値変換の対応を揃える。
...
- 永続化方式ごとの変換処理は各方式の `converter` package に置く。共通 converter を新設する場合は、MyBatis / Doma / jOOQ で本当に共有できる責務か確認する。
...
- OpenAPI のエラーレスポンスや request body の `examples` は、元 Java 版と同等の内容を維持する。Kotlin では `io.swagger.v3.oas.annotations.media.ExampleObject` を import し、`examples = [ExampleObject(...)]`、複数例は `examples = [ExampleObject(...), ExampleObject(...)]` で記述する。
- OpenAPI annotation を変更した場合は、少なくとも `./gradlew compileKotlin` で annotation 構文を確認する。API 動作や validation に関わる変更なら `./gradlew test` も実行する。
...
- 現在のデフォルト profile は `application.yaml` の `spring.profiles.default: doma` とする。JPA 用 profile や `application-jpa.yaml` は追加しない。
...
- `PurchaseInvoiceType` の値を変更する場合は、DB の `check_purchase_invoice_type` 制約、初期データ、MyBatis `PurchaseInvoiceTypeHandler`、Doma `@Domain`、jOOQ 側の `getValue()` / `of()` を使った値変換の整合性を確認する。
- `BookStockMovementType` と `BookStockMovementSourceType` は在庫増減履歴の共有ドメイン型として扱う。
- `BookStockMovementType` / `BookStockMovementSourceType` の値を変更する場合は、DB の `check_book_stock_movement_type` / `check_book_stock_movement_source_type` 制約、初期データ、MyBatis TypeHandler、Doma `@Domain`、jOOQ 側の `getValue()` を使った値変換の整合性を確認する。
...
- `src/main/kotlin/com/example/demo/openbd/generated` 配下は OpenAPI Generator 生成コードなので直接編集しない。OpenBD API 仕様や生成設定を変更する場合は `src/main/resources/openapi/openbd_api_spec.yaml`、`build.gradle` の `openApiGenerate` / `syncOpenBdGeneratedSources`、`OpenBdClientConfig`、`OpenBdProperties`、`OpenBdClientConfigTest` を確認する。
- `compileKotlin` と `kaptGenerateStubsKotlin` は `generateJooq` と `syncOpenBdGeneratedSources` に依存する。通常のビルドでも jOOQ 生成コードと OpenBD 生成コードが更新される可能性があるため、生成差分を確認する。
...
- OpenBD 生成コードを更新する場合は `./gradlew syncOpenBdGeneratedSources` を使い、`src/main/kotlin/com/example/demo/openbd/generated` の差分を確認する。

## Service と例外

- `BooksOperationService` は MyBatis / Doma / jOOQ 共通の Service インターフェースとして扱う。
- Service インターフェースを変更する場合は、MyBatis / Doma / jOOQ の3実装をすべて確認する。
- `PurchaseOperationService` は MyBatis / Doma / jOOQ 共通の仕入登録 Service インターフェースとして扱う。
...
- jOOQ 実装は `src/main/kotlin/com/example/demo/jooq` 配下に置く。手書きの SQL / DSL 組み立ては `jooq/dsl`、Service は `jooq/service`、変換は `jooq/converter`、参照存在チェックは `jooq/validator` の役割に合わせる。
- jOOQ 生成コードは `src/main/kotlin/com/example/demo/jooq/generated` 配下に出力する。生成コードを直接編集しない。
- `BookOperationConverterMybatis` / `BookOperationConverterDoma` / `BookOperationConverterJooq` は永続化方式ごとの取得結果を `BookResponse` / `BookPageResponse` 用の DTO へ変換する責務に限定する。
- `PurchaseOperationConverterMybatis` / `PurchaseOperationConverterDoma` / `PurchaseOperationConverterJooq` は仕入登録用 Entity / row、明細金額、伝票金額、在庫 Entity / row、在庫増減履歴 Entity / row、response DTO への変換を扱う。
...
- 仕入登録では伝票、明細を登録し、MyBatis は `BookStockCustomMapper.selectByStoreIdAndBookIdWithWriteLock`、Doma は `BookStockCustomDao.selectByStoreIdAndBookIdWithWriteLock`、jOOQ は `PurchaseOperationDsl` の `forUpdate().noWait()` で在庫行をロックしてから新規作成または数量加算する。
...
- Kotlin の Bean Validation annotation は対象に注意する。DTO property には `@field:NotNull` など field target を使い、コンストラクタ引数や test helper の `data class` でも validation が実際に発火することを確認する。
...
API、Security、DB 設定、MyBatis / Doma / jOOQ の実装切り替えを変更した場合は、必要に応じて `./gradlew bootRun` で起動確認し、curl または Swagger UI / Scalar で対象エンドポイントを確認する。

手書き Java の混入確認が必要な場合は、以下が 0 件であることを確認する。

```shell
rg --files src | rg '\.java$' | rg -v '/(generated|generator)/'
```
