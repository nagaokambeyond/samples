...
- 現在のスキーマには `publisher`、`book_genre`、`book`、`supplier`、`store`、`purchase_invoice`、`purchase_invoice_detail`、`book_stock`、`book_stock_movement` があります。
...
- jOOQ 生成コードには `book_stock_movement` も含まれます。jOOQ 側では在庫増減種別を DB 上の `Integer` として扱い、保存時は `BookStockMovementType#getValue()` / `BookStockMovementSourceType#getValue()` を使います。
...
- `BookWithPublisherName` は `publisherName`、`genreName`、`isbn`、`bookStockList` を含みます。取得・検索 SQL では `publisher`、`book_genre`、`book_stock`、`store` の結合を維持してください。
...
- `BookDao` / `PublisherDao` / `BookGenreDao` / `SupplierDao` / `StoreDao` / `PurchaseInvoiceDao` / `PurchaseInvoiceDetailDao` / `BookStockDao` / `BookStockMovementDao` は Doma CodeGen の生成 DAO です。生成元スキーマとの整合性を維持してください。
...
- `BookMapper` / `PublisherMapper` / `BookGenreMapper` / `SupplierMapper` / `StoreMapper` / `PurchaseOrderMapper` / `PurchaseOrderDetailMapper` / `BookStockMapper` / `BookStockMovementMapper` は MyBatis Generator の生成 Mapper です。生成元スキーマとの整合性を維持してください。
...
- `BookWithStockRowProjection` は `publisherName`、`genreName`、`isbn`、在庫・店舗表示用の行項目を含みます。取得・検索 query では `publisher`、`book_genre`、`book_stock`、`store` の結合を維持してください。
...
- `book`、`publisher`、`book_genre`、`supplier`、`store`、`purchase_invoice`、`purchase_invoice_detail`、`book_stock`、`book_stock_movement` テーブルのカラムを変更する場合は、以下の整合性を確認してください。
  - JPA Entity
  - JPA Repository
